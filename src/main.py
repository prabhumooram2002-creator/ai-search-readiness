...
{"✅ Phase 2 entity extraction complete!" if entity_count > 0 else "⚠️  Phase 2 skipped (graph not available)"}
...
async def run_audit(start_url: str) -> dict:
    ...
    if not success:
        ...
        sys.exit(1)
    print(f"   ✅ {len(success)} pages crawled")
    stats["crawl_results"] = crawl_results

    # ── Phase 1: Chunk ─────────────────────────────────────────────────
    print(f"\n📦 [Phase 1] Chunking {len(success)} pages ...")
    all_chunks = chunk_pages(crawl_results)
    print(f"   ✅ {len(all_chunks)} chunks created")
    stats["chunks"] = all_chunks

    # ── Phase 1: Embed ─────────────────────────────────────────────────
    print(f"\n🧠 [Phase 1] Embedding {len(all_chunks)} chunks (NVIDIA NIM free tier) ...")
    ...
    # ── Phase 1: Store ─────────────────────────────────────────────────
    print(f"\n💾 [Phase 1] Storing in ChromaDB ...")
    vs = VectorStore()
    vs.add_chunks(all_chunks, embeddings)
    print(f"   ✅ {vs.count()} total chunks in vector store")
    ...
    try:
        print(f"\n🔗 [Phase 2] Connecting to Kuzu graph ...")
        graph = get_graph()
        graph.verify_connection()
        graph_available = True
        stats["graph_available"] = True
        print(f"   ✅ Kuzu graph connected")

        print(f"\n🏗️  [Phase 2] Extracting entities + relationships ...")
        entities, relationships = await extract_from_chunks(all_chunks, use_cache=True)
        print(f"   ✅ {len(entities)} entities, {len(relationships)} relationships extracted")
        ...
        if entities or relationships:
            ...
            print(f"   ✅ {entity_count} entities, {rel_count} relationships in graph")

            stats["entities"] = entities
            stats["relationships"] = relationships

    except Exception as e:
        print(f"\n⚠️  [Phase 2] Graph not available: {e}")
...
def interactive_loop():
    ...
    print("\n💬 Ask me anything about the indexed site.\n")
    ...
    while True:
        ...
        if cmd == "status":
            vs = VectorStore()
            print(f"\n📊 Vector store: {vs.count()} chunks\n")
            try:
                graph = get_graph()
                gs = graph.get_stats()
                print(f"📊 Graph: {gs['nodes']} entities, {gs['relationships']} relationships\n")
            except Exception as e:
        ...
        if cmd == "reset":
            ...
            try:
                reset_graph()
                print("🗑️  Vector store + graph cleared.\n")
            except Exception:
                print("🗑️  Vector store cleared (graph not available).\n")
            continue
        ...
        if result.graph_path:
            print(f"\n🕸️  Graph path: {result.graph_path}")
        if result.missing_relationship:
            print(f"⚠️  Missing: {result.missing_relationship}")
...
def main():
    ...
    if cmd == "audit":
        ...
        print(f"\n💾 Run `gap-report` to generate the full HTML report.\n")
        ...
        try:
            ...
            print(f"📊 Graph: not configured ({e})")
